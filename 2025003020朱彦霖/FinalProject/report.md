...
### 10.5 错误 / 空状态 `screenshots/empty.png`(选做补充)

说明:展示空状态:`EmptyState` 组件,提示"还没有专注记录,去开始第一次吧"+"去计时"按钮。
...
* **运行步骤**:
  ...
  2. 进入项目目录:`cd homework/FinalProject/FocusTimer`
  3. 用 Android Studio 打开 `homework/FinalProject/FocusTimer` 目录
  4. 等待 Gradle 同步完成(首次较慢,需下载 Compose BOM、Room、Retrofit 等依赖)
  5. 连接 Android 7.0+ 模拟器或真机,点击 Run
  6. (可选)打包命令:`./gradlew assembleDebug`,产物在 `app/build/outputs/apk/debug/`
